#include "visuals.hpp"

visuals::visuals(const CCSPlayerPawn* _pawn) : pawn(_pawn) {}

void visuals::render_name() const {
	if (pawn->m_iHealth() <= 0)
		return;

	vector_3d head_position = { };
	if ((head_position = CBone::get_bone_position(pawn, Head)).is_zero())
		return;

	head_position.z += 15.0f;

	vector_2d text_position = { };
	if (!math::world_to_screen(head_position, text_position))
		return;

	const char* name = pawn->m_sSanitizedPlayerName();
	im_vec_2 text_size = ref_to_draw.get_text_size(fonts.get("MuseoSans-500_12"), name);
	ref_to_draw.draw_text(im_vec_2(text_position.x - text_size.x * 0.5f, text_position.y - text_size.y), ImColor(255, 255, 255, 255), fonts.get("MuseoSans-500_12"), name);
}

void visuals::render_box() const {
	if (pawn->m_iHealth() <= 0)
		return;

	vector_3d m_vecMins = pawn->m_pCollision()->m_vecMins();
	vector_3d m_vecMaxs = pawn->m_pCollision()->m_vecMaxs();

	float collision_height = m_vecMaxs.z - m_vecMins.z;

	vector_3d center = pawn->m_vOldOrigin();
	center.z = collision_height * 0.5f;

	float z_offset = 58.0f;

	vector_3d head_position = { };
	if ((head_position = CBone::get_bone_position(pawn, Head)).is_zero())
		return;

	vector_3d origin = pawn->m_vOldOrigin();
	float height = std::round(head_position.z - origin.z);
	float width = height / 4.0f;

	std::array<vector_3d, 4> vertices_3d = {
		vector_3d(origin.x, origin.y - width, origin.z + height + 10.0f),	// Upper-Left
		vector_3d(origin.x, origin.y + width, origin.z + height + 10.0f),	// Upper-Right
		vector_3d(origin.x, origin.y + width, origin.z),					// Lower-Left
		vector_3d(origin.x, origin.y - width, origin.z)						// Lower-Right
	};

	std::array<vector_2d, 4> vertices_2d;
	for (size_t i = 0; i < 4; i++) {
		if (!math::world_to_screen(vertices_3d[i], vertices_2d[i]))
			return;
	}

	for (size_t i = 0; i < 3; i++) {
		ref_to_draw.draw_line(im_vec_2(vertices_2d[i].x, vertices_2d[i].y), im_vec_2(vertices_2d[i + 1].x, vertices_2d[i + 1].y), ImColor(255, 255, 255, 255), 0.8f);

		if (i == 2)
			ref_to_draw.draw_line(im_vec_2(vertices_2d[i + 1].x, vertices_2d[i + 1].y), im_vec_2(vertices_2d[0].x, vertices_2d[0].y), ImColor(255, 255, 255, 255), 0.8f);
	}
}

void visuals::render_health() const {
	if (pawn->m_iHealth() <= 0)
		return;

	vector_3d head_position = { };
	if ((head_position = CBone::get_bone_position(pawn, Head)).is_zero())
		return;

	head_position.y -= 25.0f;
	head_position.z += 5.0f;
	vector_2d bar_position_start = { };
	if (!math::world_to_screen(head_position, bar_position_start))
		return;

	vector_2d bar_position_end = { };

	vector_3d origin = pawn->m_vOldOrigin();
	if (!math::world_to_screen(vector_3d(origin.x, origin.y - 22.0f, origin.z), bar_position_end))
		return;

	float bar_width = 7.0f, health_bar_width = 3.0f;
	ref_to_draw.draw_rect(im_vec_2(bar_position_start.x, bar_position_start.y), im_vec_2(bar_position_end.x, bar_position_end.y), ImColor(34, 34, 34, 255));
	ImColor health_color = { 1.0f - (float)pawn->m_iHealth() / 100.0f, (float)pawn->m_iHealth() / 100.0f, 0.0f, 1.0f };
	//ref_to_draw.draw_rect(im_vec_2(bar_position_start.x, bar_position_start.y), im_vec_2(bar_position_end.x, bar_position_end.y), health_color);
}

void visuals::render_skeleton() const {
	if (pawn->m_iHealth() <= 0)
		return;

	vector_3d bone_position_start = { }, bone_position_end = { };
	vector_2d line_start = { }, line_end = { };

	for (const auto& group : bones)
		for (size_t i = 0; i < group.size() - 1; i++) {
			if ((bone_position_start = CBone::get_bone_position(pawn, group[i])).is_zero())
				return;

			if ((bone_position_end = CBone::get_bone_position(pawn, group[i + 1])).is_zero())
				return;

			if (math::world_to_screen(bone_position_start, line_start) && math::world_to_screen(bone_position_end, line_end)) {
				ref_to_draw.draw_line(im_vec_2(line_start.x, line_start.y), im_vec_2(line_end.x, line_end.y), ImColor(255, 255, 255, 255), 0.8f);
				continue;
			}

			return;
		}
}