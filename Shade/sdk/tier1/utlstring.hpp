...
public:
	...
	void		Set(const char* pValue) {
		...
		const size_t len = std::strlen(pValue);
		m_pString = new char[len + 1];
		std::memcpy(m_pString, pValue, len + 1);
	}
	...
	void ToUpperFast() {
		...
		{
			unsigned char c = static_cast<unsigned char>(*p);
			*p = static_cast<char>(std::toupper(c));
		}
	}
