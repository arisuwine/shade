...
// hash a uint32_t into a uint32_t
FORCEINLINE uint32_t HashIntAlternate(uint32_t n)
{
	n = (n + 0x7ed55d16) + (n << 12);
	n = (n ^ 0xc761c23c) ^ (n >> 19);
	n = (n + 0x165667b1) + (n << 5);
...
uint32_t MurmurHash2LowerCase(char const* pString, uint32_t nSeed);
uint32_t MurmurHash2LowerCase(char const* pString, int nLength, uint32_t nSeed);

uint32_t MurmurHash3_32(void const* key, size_t len, uint32_t seed, bool bCaselessStringVariant = false);
